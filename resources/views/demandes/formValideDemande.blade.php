                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <form class=" card-body row g-3 " method="POST"
                                action="{{ route('validerDemande', ['id'=>$demande->id, 'type'=>'voiture']) }}">
                            @csrf
                            <div class="col-12">
                            ...
                            @if ($demande->addchauffeur)
                                <div class="col-12">
                                    ...
                                    <select class="form-control selectpicker" multiple data-live-search="true" name="chauffeurs[]" id="chauffeurs">
                                        @if ($chauffeurs->count() > 0)
                                            @foreach ($chauffeurs as $chauffeur)
                                                @if ($chauffeur->user->structure_id == Auth::user()->structure_id)
                                                    <option value="{{ $chauffeur->user->id }}">{{ $chauffeur->user->name }}</option>
                                                @endif
                                            @endforeach
                                        @else
